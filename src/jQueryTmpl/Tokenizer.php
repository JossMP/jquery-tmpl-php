...
class Tokenizer
{
    ...
    public function __construct(\jossmp\jQueryTmpl\Token\Factory $tokenFactory)
    {
        $this->_tokenFactory = $tokenFactory;
    }

    public function addTag(\jossmp\jQueryTmpl\Tag $tag)
    {
        $this->_tags[] = $tag;
        return $this;
    }
    ...
    private function _createTokensFromMarkers($template, array $markers)
    {
        ...
        if ($currNest != 0) {
            throw new \jossmp\jQueryTmpl\Tokenizer\Exception('jQuery Template can not be tokenized, there exists an unclosed tag.');
        }
