...
class Parser
{
    ...
    public function __construct(\jossmp\jQueryTmpl\Element\Factory $elementFactory)
    {
    ...
    public function parse(array $tokens)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
             ...
             */
            if ($tokens[$i] instanceof \jossmp\jQueryTmpl\Token\TypeBlock) {
                $startToken = $tokens[$i];

                if (!$startToken->isBlockStart()) {
                    throw new \jossmp\jQueryTmpl\Parser\Exception('A unmatched block end tag has been encountered.');
                }

                $blockNest = $startToken->getLevel();
                $endToken = '\jossmp\jQueryTmpl\Token\\' . $startToken->getBlockEndToken();
             ...
             *  create method.
             */
            if ($tokens[$i] instanceof \jossmp\jQueryTmpl\Token\TypeControl) {
             ...
             *  create method.
             */
            if ($tokens[$i] instanceof \jossmp\jQueryTmpl\Token\TypeInline) {
            ...
            throw new \jossmp\jQueryTmpl\Parser\Exception('Uncaught token type detected.');
        }
