...
class Data
{
    ...
    public function __construct($data, \jossmp\jQueryTmpl\Data\Factory $dataFactory)
    {
    ...
    private function _parseJsNotation($js)
    {
        ...
        foreach ($loc as $idx => $name) {
            ...
            if (!preg_match('/^([a-z_$][0-9a-z_$]*)(\[.*\])?$/i', $name, $match)) {
                throw new \jossmp\jQueryTmpl\Data\Exception('\jossmp\jQueryTmpl\Data can not evaluate expressions.');
            }
